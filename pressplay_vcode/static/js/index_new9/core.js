...
function bindSegmentGridInteractions() {
    document.querySelectorAll(".segment-box").forEach(box => {
        ...
        box.addEventListener("click", function () {
            // Fail-safe: always seek video to selected segment start on click.
            try {
                const start = parseFloat(this.dataset.start || "NaN");
                if (Number.isFinite(start)) {
                    if (typeof window.seekVideoToTime === "function") {
                ...
                console.warn("Segment click seek fallback failed:", seekErr);
            }
...
window.deleteSelectedSegment = async function() {
    if (window.__deleteSegmentInFlight) {
        return;
    }
    ...
    try {
        window.__deleteSegmentInFlight = true;
        ...
        if (typeof refreshSegmentsOnly === 'function') {
            await refreshSegmentsOnly({ background: false, reason: "delete" });
        } else if (typeof window.scheduleSegmentsRefresh === "function") {
            window.scheduleSegmentsRefresh("delete", 0);
        }
        ...
        window.__deleteSegmentInFlight = false;
    }
};

let __segmentsRefreshTimer = null;
let __segmentsRefreshInFlight = false;
let __segmentsRefreshQueuedReason = "";

function formatSegmentTimeForUi(seconds) {
    const n = Number(seconds);
    if (!Number.isFinite(n)) return "00:00:00:00";
    if (typeof formatTimecode === "function") {
        return formatTimecode(n);
    }
    return n.toFixed(2);
}

function refreshSegmentTimecodeCell(segmentBox, index) {
    const timecodeEl = segmentBox.querySelector(".timecode");
    if (!timecodeEl) return;
    const start = Number(segmentBox.dataset.start);
    const end = Number(segmentBox.dataset.end);
    timecodeEl.innerHTML = `${index + 1}<br />${formatSegmentTimeForUi(start)}<br />${formatSegmentTimeForUi(end)}`;
}

function reindexSegmentGridDom() {
    const boxes = Array.from(document.querySelectorAll("#segment-grid .segment-box"));
    boxes.forEach((box, i) => {
        ...
        box.querySelectorAll(".live-editor[id]").forEach((el) => {
            if (el.id.endsWith("-en")) el.id = `live-${i}-en`;
            if (el.id.endsWith("-ar")) el.id = `live-${i}-ar`;
        });

        refreshSegmentTimecodeCell(box, i);
    });
}

window.optimisticDeleteSegment = function optimisticDeleteSegment(segmentIndex) {
    console.warn("optimisticDeleteSegment disabled; using server-authoritative refresh");
    if (typeof window.scheduleSegmentsRefresh === "function") {
        window.scheduleSegmentsRefresh("delete", 0);
    }
    return false;
};

window.optimisticSplitSegment = function optimisticSplitSegment(segmentIndex, splitTime) {
    console.warn("optimisticSplitSegment disabled; using server-authoritative refresh");
    if (typeof window.scheduleSegmentsRefresh === "function") {
...
window.optimisticInsertSegment = function optimisticInsertSegment(insertIndex, data = {}) {
    console.warn("optimisticInsertSegment disabled; using server-authoritative refresh");
    if (typeof window.scheduleSegmentsRefresh === "function") {
...
window.scheduleSegmentsRefresh = function scheduleSegmentsRefresh(reason = "update", delayMs = 80) {
    if (__segmentsRefreshTimer) {
        clearTimeout(__segmentsRefreshTimer);
    }
    __segmentsRefreshTimer = setTimeout(async () => {
        if (__segmentsRefreshInFlight) {
            __segmentsRefreshQueuedReason = reason;
            return;
        }
        __segmentsRefreshInFlight = true;
        window.__segmentsRefreshInFlight = true;
        try {
            await refreshSegmentsOnly({ background: true, reason });
        } catch (err) {
            console.warn("Background segment refresh failed:", err);
        } finally {
            __segmentsRefreshInFlight = false;
            window.__segmentsRefreshInFlight = false;
            if (__segmentsRefreshQueuedReason) {
                const nextReason = __segmentsRefreshQueuedReason;
                __segmentsRefreshQueuedReason = "";
                window.scheduleSegmentsRefresh(nextReason, 150);
            }
        }
    }, Math.max(0, Number(delayMs) || 0));
};

async function refreshSegmentsOnly(options = {}) {
    const opts = {
        background: false,
        reason: "",
        ...options
    ...
    const projectName = window.currentProject || boot.project || "";
    const episodeName = window.currentEpisode || boot.episode || "";
    const url = `/editor/${encodeURIComponent(projectName)}/${encodeURIComponent(episodeName)}?t=${Date.now()}`;

    const previouslyActive =
        document.querySelector(".segment-box.active") ||
        (typeof activeBox !== "undefined" ? activeBox : null);
    const previouslyActiveIndex = previouslyActive?.dataset?.index ?? localStorage.getItem(LAST_SEGMENT_KEY);
    ...
    // keep UI interactive and avoid expensive audio/file work in handleSegmentSelection.
    if (typeof loadDoneState === "function") {
        void loadDoneState().catch(err => console.warn("Background done-state refresh failed:", err));
    }

    let targetBox = null;
    if (previouslyActiveIndex !== null && previouslyActiveIndex !== undefined) {
        targetBox = document.querySelector(`.segment-box[data-index="${previouslyActiveIndex}"]`);
    }
    if (!targetBox) {
        targetBox = document.querySelector(".segment-box");
    }

    document.querySelectorAll(".segment-box.active").forEach(el => el.classList.remove("active"));
    if (targetBox) {
        targetBox.classList.add("active");
        activeBox = targetBox;
        activeSegmentIndex = Number.parseInt(targetBox.dataset.index || "-1", 10);
    }
}
...
async function preloadMediaToTemp() {
    ...
    const videoUrl = `/video_stream/${encodeURIComponent(project)}/${encodeURIComponent(episode)}?t=${Date.now()}`;
...
function disableUserInterface(disabled) {
    ...
    elementsToDisable.forEach(selector => {
        document.querySelectorAll(selector).forEach(element => {
            ...
            async function saveSubtitle(index, lang, text, speaker, start, end) {
                if (typeof window.isEpisodeEditRestricted === "function" && window.isEpisodeEditRestricted()) {
                    return {
                        status: "locked",
                        message: `Editing is locked by ${window.__episodeEditRestrictedBy || "another user"}.`,
                        locked_by: window.__episodeEditRestrictedBy || ""
                    };
                }
                const formData = new FormData();
        ...
        if (!sessionStorage.getItem("subtitleUserId")) {
            const uid = "User " + Math.floor(1000 + Math.random() * 9000);
            sessionStorage.setItem("subtitleUserId", uid);
        }
        if (!sessionStorage.getItem("subtitleSessionId")) {
            const sid = `sess_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
            sessionStorage.setItem("subtitleSessionId", sid);
        }
        ...
        window.__episodeEditRestricted = false;
        window.__episodeEditRestrictedBy = "";
        window.isEpisodeEditRestricted = function isEpisodeEditRestricted() {
            return !!window.__episodeEditRestricted;
        };
        window.setEpisodeEditRestriction = function setEpisodeEditRestriction(locked, lockedBy = "") {
            window.__episodeEditRestricted = !!locked;
            window.__episodeEditRestrictedBy = String(lockedBy || "").trim();

            const restrictionMsg = window.__episodeEditRestricted
                ? `View-only mode: editing locked by ${window.__episodeEditRestrictedBy || "another user"}`
                : "Editing unlocked";
            setInsertStatus(restrictionMsg, window.__episodeEditRestricted ? "#dc3545" : "#28a745");

            const allowedById = new Set(["preview-btn"]);
            const targets = document.querySelectorAll(
                "textarea.edit-field, .speaker-dropdown, #segment-done-checkbox, #start-recording, #stop-recording, #tts-btn, #tts-load-selected-wave, #vc-btn, button[onclick*='splitSegmentAtCurrentTime'], button[onclick*='deleteSelectedSegment'], button[onclick*='openInsertModal']"
            );
            targets.forEach((el) => {
                const id = (el.id || "").trim();
                if (allowedById.has(id)) return;
                if (window.__episodeEditRestricted) {
                    if (el.dataset.lockPrevDisabled === undefined) {
                        el.dataset.lockPrevDisabled = String(!!el.disabled);
                    }
                    el.disabled = true;
                } else if (el.dataset.lockPrevDisabled !== undefined) {
                    el.disabled = el.dataset.lockPrevDisabled === "true";
...
async function playFullVideoWithGeneratedWavsSynced() {
    ...
    const activeStart = (typeof activeBox !== "undefined" && activeBox)
        ? parseFloat(activeBox.dataset.start || "NaN")
        : NaN;
    const currentVideoTime = Number.isFinite(activeStart) ? activeStart : (video.currentTime || 0);
