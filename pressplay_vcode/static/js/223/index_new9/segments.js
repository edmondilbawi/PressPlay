// Segment insertion functionality
        ...
        async function insertNewSegment(start, duration, speaker, englishText, arabicText) {
            ...
            try {
                ...
                console.log('Sending insert request:', {
                    project, episode, start, end, speaker, englishText, arabicText
                });
        ...
        function splitSegmentAtCurrentTime() {
            ...
            const activeSegment = findSegmentAtTime(currentTime);
            
            if (!activeSegment) {
                alert("No segment found at current time. Please select a segment first.");
                return;
            }
            ...
            if (currentTime <= segmentStart || currentTime >= segmentEnd) {
                alert("Current time must be within the segment duration");
                return;
            }
            
            if (confirm(`Split segment at ${formatTimecode(currentTime)}?`)) {
        ...
        async function splitSegment(segmentIndex, splitTime) {
            try {
                ...
                console.log('Sending split request:', {
                    project, episode, segmentIndex, splitTime
                });
